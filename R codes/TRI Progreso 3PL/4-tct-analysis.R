...
#   - TCT-LEC-Mes04-Junio.xlsx      (RPbis corregido por opción)
#   - TCT-MAT-Mes04-Junio.xlsx
#   - Distractores-LEC-Mes04-Junio.xlsx  (análisis de distractores por tercil)
#   - Distractores-MAT-Mes04-Junio.xlsx
# =============================================================================
    ...
    option_rows <- lapply(options, function(opt) {
      ...
      data.frame(
        ...
        Discrimina  = ifelse(!is.na(correct) & opt == correct,
                             ifelse(freq_alto - freq_bajo > 0.1, "Si", "No"), ""),
        stringsAsFactors = FALSE
      )
    })
...
pruebas_meta <- data.frame(
  ItemCodigo     = trimws(pruebas_raw$ItemCodig),         # <- "ItemCodig" sin 'o'
...
lapply(c("LEC", "MAT"), function(subject) {
  ...
  # --- Guardar TCT (RPbis) ---
  tct_path <- paste0("./results/TCT/TCT-", subject, "-Mes04-Junio.xlsx")
  writexl::write_xlsx(tct_by_grade, tct_path)
  message("  Guardado: ", tct_path)
  
  # --- Guardar Distractores ---
  dist_path <- paste0("./results/TCT/Distractores-", subject, "-Mes04-Junio.xlsx")
