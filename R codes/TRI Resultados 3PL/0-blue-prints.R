...
# Ítems detectados como inválidos (mal calibrados/errores) a remover ----
# Estructura: lista nombrada por Area, cada elemento es una lista nombrada por Grado
# con el/los códigos de ítem (ItemCodig) a excluir de la calibración TRI.
items_invalidos <- list(
  LEC = list("2do" = c("LEC4287")),
  MAT = list()
)
...
# Remover del BluePrint los ítems marcados como inválidos ----
for (area in names(items_invalidos)) {
  for (grado in names(items_invalidos[[area]])) {
    items_to_drop <- items_invalidos[[area]][[grado]]
    idx_drop <- items_pruebas$Area == area &
      items_pruebas$Grado == grado &
      items_pruebas$Item %in% items_to_drop
    if (any(idx_drop)) {
      print(paste0("BluePrint: removiendo ítem(s) invalido(s) - Area: ", area,
                   " - Grado: ", grado, " - Item(s): ", paste(items_to_drop, collapse = ", ")))
    }
    items_pruebas <- items_pruebas[!idx_drop, ]
  }
}
...
process_exam <- function(
    ...
    base_path = "./data/raw/Reporte CML 2 puntajes/", is_nominal = FALSE) {
...
# Remueve, por grado, las columnas de ítem marcadas como inválidas en `items_invalidos`
drop_invalid_items <- function(list_df, prefix = "LEC") {
  if (is.null(items_invalidos[[prefix]]) || length(items_invalidos[[prefix]]) == 0) {
    return(list_df)
  }
  for (grado in names(items_invalidos[[prefix]])) {
    if (grado %in% names(list_df)) {
      cols_drop <- intersect(items_invalidos[[prefix]][[grado]], colnames(list_df[[grado]]))
      if (length(cols_drop) > 0) {
        print(paste0("IRT-", prefix, ": removiendo columna(s) invalida(s) del grado ",
                     grado, " - Item(s): ", paste(cols_drop, collapse = ", ")))
        list_df[[grado]] <- list_df[[grado]][, setdiff(colnames(list_df[[grado]]), cols_drop)]
      }
    }
  }
  return(list_df)
...
process_resp <- function(
    Pruebas_SV, prefix = "LEC",
    base_path = "./data/raw/Reporte CML 2 puntajes/", min_duration = 5, is_nominal = FALSE) {
...
writexl::write_xlsx(process_resp(Pruebas_SV, "MAT") |> drop_invalid_items("MAT"),
                    "./data/processed/IRT-MAT.xlsx")
writexl::write_xlsx(process_resp(Pruebas_SV, "LEC") |> drop_invalid_items("LEC"),
                    "./data/processed/IRT-LEC.xlsx")

writexl::write_xlsx(
  process_resp(Pruebas_SV, prefix = "MAT",
               base_path = "./data/raw/Reporte CML 2 letras/", is_nominal = TRUE) |>
    drop_invalid_items("MAT"),
