...
lapply(c("MAT","LEC"), FUN = function(subject) {
  ...
  resultados <- lapply(lgrades, FUN = function(grade) {
    ...
    df <- read_excel(paste0("./data/processed/",subject,"-exam_pendiente.xlsx"), sheet = grade)
  ...
  writexl::write_xlsx(resultados, paste0("./results/",subject,"-resultados_pendiente-Mes4-Junio-2026.xlsx"))
