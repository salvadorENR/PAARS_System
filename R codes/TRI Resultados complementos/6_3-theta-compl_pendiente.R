...
lapply(c("MAT","LEC"), FUN = function(subject) {
  ...
  toSave <- lapply(lgrades, FUN = function(grade) {
    ...
    if (!file.exists(rds_file)) {
      ...
      exam_dat <- read_excel(paste0("./data/processed/",subject,"-exam_pendiente.xlsx"), sheet = grade)
      
      exam_with_theta <- exam_dat
      for (key in names(result$lresult_mirt)) {
        if (length(result$lresult_mirt[[key]]) > 0) {
          
          cnames <- result$lresult_mirt[[key]]$items
          resp <- exam_dat[, cnames]
          theta_df <- as.data.frame(fscores(result$lresult_mirt[[key]]$md, response.pattern = resp))
  ...
  writexl::write_xlsx(c(
    ...
    paste0("./results/",subject,"-resultados_compl_pendiente-Mes4-Junio-2026.xlsx"))
