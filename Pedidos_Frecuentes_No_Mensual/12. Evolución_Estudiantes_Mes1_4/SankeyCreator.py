...
# NIVEL_ORD uses score-order (Crítico=0 … Excelente=4) for logic/comparisons
NIVELES   = ["Crítico", "Bajo", "Medio", "Bueno", "Excelente"]
NIVEL_ORD = {n: i for i, n in enumerate(NIVELES)}

# NIVELES_DISPLAY is reversed for top-to-bottom visual order in the Sankey
# (Excelente on top, Crítico on bottom — most intuitive reading)
NIVELES_DISPLAY = ["Excelente", "Bueno", "Medio", "Bajo", "Crítico"]

# Colors and links ordered to match NIVELES_DISPLAY (Excelente→Crítico)
COLOR_NODO = ["#065f46", "#84cc16", "#facc15", "#ff8c2e", "#991b1b"]
COLOR_LINK = [
    "rgba(  6, 95,  70, 0.35)",
...
def calc_y_centers(conteos_nivel):
    totals   = [conteos_nivel.get(n, 0) for n in NIVELES_DISPLAY]
    pad_frac = 0.04
...
def _construir_subtitulo_trayectorias(tray_counts, n_est, orden_etiq,
                                       desc_poblacion, df_mat=None):
    """
    Genera el HTML del subtítulo con conteos, porcentajes y desglose por nivel.
    Cada categoría de trayectoria ocupa su propia línea para evitar cortes.
    El desglose por nivel muestra % sobre el TOTAL de estudiantes (n_est).
    """
    # Use the last Nivel_* column (most recent month) as the reference level
    nivel_ref_col = None
    if df_mat is not None:
        nivel_cols = [c for c in df_mat.columns if c.startswith("Nivel_")]
        if nivel_cols:
            nivel_ref_col = nivel_cols[-1]

    # Line 1: population description
    # Lines 2+: one line per trajectory category + its level breakdown below
    html_lines = [
        f"<span style='font-size:12px;color:#333;font-weight:bold'>"
        f"{desc_poblacion}</span>"
    ]

    for et in orden_etiq:
        cnt = tray_counts.get(et, 0)
        if cnt == 0:
            continue
        pct = cnt / n_est * 100
        ...
        cat_html = (
            f"<span style='font-size:11px;color:#1a2b4c;font-weight:bold'>"
            f"{et}:</span>"
            f"<span style='font-size:11px;color:#444'> {cnt:,} ({pct:.1f}%)</span>"
        )

        # Level breakdown: one entry per level, smaller font, indented
        if df_mat is not None and nivel_ref_col is not None:
            df_tray = df_mat[df_mat["Trayectoria"] == et]
            nivel_breakdown = df_tray[nivel_ref_col].value_counts()
            partes = []
            for nivel in NIVELES_DISPLAY:
                nc = nivel_breakdown.get(nivel, 0)
                if nc > 0:
                    partes.append(
                        f"<span style='font-size:10px;color:#666'>"
                        f"{nivel}: {nc:,} ({nc / n_est * 100:.1f}%)</span>"
                    )
            if partes:
                breakdown_html = (
                    f"<span style='font-size:10px;color:#999'>&nbsp;&nbsp;&nbsp;↳ </span>"
                    + "<span style='font-size:10px;color:#bbb'> | </span>".join(partes)
                )
                cat_html += "<br>" + breakdown_html

        html_lines.append(cat_html)

    # Join all category blocks with a thin separator line
...
def dibujar_sankey(df_wide, meses_req, materia, sufijo, dir_sankey,
    ...
    for i, mes in enumerate(meses_req):
        ...
        for j, nivel in enumerate(NIVELES_DISPLAY):
    ...
    for i in range(len(meses_req) - 1):
        ...
        for _, row in flujos.iterrows():
            ...
            # Map level name → position in NIVELES_DISPLAY for node index
            DISP_ORD = {n: i for i, n in enumerate(NIVELES_DISPLAY)}
            si = oo + DISP_ORD[row[col_a]]
            ti = od + DISP_ORD[row[col_b]]
            sources.append(si); targets.append(ti)
            values.append(int(row["n"]))
            link_colors.append(COLOR_LINK[DISP_ORD[row[col_a]]])

    # ── Subtítulo ─────────────────────────────────────────────────────────
    tray_counts = df_mat["Trayectoria"].value_counts()
    subtitulo   = _construir_subtitulo_trayectorias(
        tray_counts, n_est, orden_etiq, desc_poblacion, df_mat=df_mat)
    ...
    # Top margin scales with number of trajectory categories present
    n_tray_active = sum(1 for et in orden_etiq
                        if df_mat["Trayectoria"].value_counts().get(et, 0) > 0)
    # Each category: ~28px for header + ~18px for breakdown = ~46px; plus base 60px
    top_margin = 60 + n_tray_active * 46
    title_y    = 1.0 - (30 / (620 + top_margin))  # keep title just inside top

    fig.update_layout(
        title=dict(text=titulo_completo, x=0.01, y=title_y),
        font_size=11,
        width=max(950, 820 + len(meses_req) * 160),
        height=620 + top_margin,
