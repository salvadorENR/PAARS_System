...
def calcular_metricas_resultados(df_master, items_cols):
    ...
    # --- FILTRO DE PRUEBAS VÁLIDAS ---
    # Garantiza que el motor LaTeX solo trabaje con pruebas no anuladas
    ...
    df_clean['Grado_Num'] = df_clean['Grado'].astype(str).str.extract(r'(\d+)').astype(float).fillna(0).astype(int)
    
    grados = sorted([g for g in df_clean['Grado_Num'].unique() if g >= 3])
    ...
    for grado in grados:
        ...
        for materia in ['Matemática', 'Lengua']:
            ...
            df_subset = df_clean[(df_clean['Grado_Num'] == grado) & 
                                 (df_clean['Area temática'] == materia)].copy()
            ...
            # 2. QUINTILES
            quintiles_data = []
            # Candado de seguridad: Solo calcular quintiles si hay al menos 5 alumnos
            if N_validos >= 5:
            ...
            items_materia = [col for col in items_cols if str(col).upper().startswith(prefijo) and col in df_subset.columns]
            
            item_diff = []
            for item in items_materia:
                # Convertir a numérico (1=Correcto, 0=Incorrecto/Omitido)
