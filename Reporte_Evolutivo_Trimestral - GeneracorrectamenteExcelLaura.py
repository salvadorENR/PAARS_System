...
def cargar_req027():
    """
    Loads REQ_027_1.xlsx and returns a lookup Series:
        NIE (str) → Group_ID (str)

    The file has a direct NIE column which is used as the join key.
    Both the NIE key and Group_ID are normalised to clean strings
    (no .0 suffix, no quotes, stripped whitespace) so they match
    the student NIEs from the scores files.
    """
    ...
    print(f"      Columnas detectadas en REQ_027_1: {list(df.columns)}")

    # Verify GROUP_ID exists
    if 'GROUP_ID' not in df.columns:
        print("  [!] columna 'GROUP_ID' no encontrada en REQ_027_1.xlsx.")
        print(f"      Columnas disponibles: {list(df.columns)}")
        return pd.Series(dtype=str)

    # Find NIE column — try common names
    nie_col = next(
    ...
    # Fallback: first column that isn't GROUP_ID
    if nie_col is None:
        candidates = [c for c in df.columns if c != 'GROUP_ID']
        nie_col    = candidates[0] if candidates else None

    if nie_col is None:
        print("  [!] No se identificó columna NIE en REQ_027_1.xlsx.")
        return pd.Series(dtype=str)

    print(f"  [OK] Columna NIE encontrada: '{nie_col}'")

    # Normalise NIE key: remove .0 suffix, quotes, extra whitespace
    ...
    # Drop rows where key or Group_ID is empty / NaN
    ...
    print(f"  [OK] REQ_027_1 — {len(lookup)} NIEs mapeados a Group_ID")
    print(f"      Muestra lookup: {dict(list(lookup.items())[:3])}")
    return lookup


def _aplicar_group_id(df_out, lookup_req027):
    """
    Maps Group_ID from REQ_027_1 lookup onto df_out using the NIE column
    and writes the result into CÓDIGO_SECCIÓN.
    NIE values are normalised to plain strings before matching so
    integer NIEs (e.g. 20113907) match string keys ('20113907').
    """
    if lookup_req027.empty:
        print("  [!] lookup_req027 vacío — CÓDIGO_SECCIÓN no se rellenará.")
        return df_out

    # Find NIE column in df_out
    nie_col = next(
        (c for c in df_out.columns
         if c.upper() in ('NIE', 'NIE_LIMPIO')), None)

    if nie_col is None:
        print("  [!] No se encontró columna NIE en df_out.")
        print(f"      Columnas: {list(df_out.columns)}")
        return df_out

    # Normalise: convert integers/floats to clean strings
    ...
    matched   = group_ids.notna().sum()
    total     = len(group_ids)

    print(f"  [OK] CÓDIGO_SECCIÓN: {matched}/{total} estudiantes "
          f"mapeados (clave: '{nie_col}')")

    if matched == 0:
        sample_out    = nie_key.dropna().unique()[:5].tolist()
        sample_lookup = lookup_req027.index[:5].tolist()
        print("  [!] 0 coincidencias — revisar formato de claves.")
        print(f"      Muestra claves df_out : {sample_out}")
        print(f"      Muestra claves REQ_027: {sample_lookup}")
    ...
    for ruta, etiqueta_mes in DIRECTORIOS_MESES:
        ...
        for f in glob.glob(os.path.join(ruta, "*.csv")):
            ...
            try:
                ...
                df['Grado_Num']     = df[col_grado].apply(
                ...
                df['Documento']     = (df[col_doc].astype(str)
                                       .str.replace(r'\.0$', '',
                                                    regex=True)
                                       .str.strip())
                df['Nivel_Logro']   = df[COL_SCORE_EXACTA].apply(
                    clasificar_puntaje)
...
# ==========================================
# 6. BARRAS APILADAS POR GRUPO DE GRADOS
# ==========================================
def generar_barras_por_grupo_grados(df_master, meses_labels,
                                    sufijo_archivo="B1"):
    ...
    for nombre_grupo, grados in GRUPOS_GRADOS.items():
        ...
        df_grupo['Grado_Str'] = (df_grupo['Grado_Num'].astype(str)
                                 + "° Grado")
...
# ==========================================
# 7. KDE + BOXPLOT JITTER
# ==========================================
def generar_kde_y_boxplot(df_master, meses_labels,
                          sufijo_archivo="B1", color_list=None):
...
# 8. B1 CONSOLIDADO
# ==========================================
def exportar_base_consolidada_b1(df_master, df_matricula,
                                  lookup_req027, meses_labels):
...
# ==========================================
# 10. EXCEL MAESTRO — HOJA ÚNICA
# ==========================================
def construir_excel_unico(df_merged, df_matricula,
                           meses_labels, lookup_req027):
    """
    Single-sheet Excel with ALL students from B1 and B2.
    CÓDIGO_SECCIÓN is filled from REQ_027_1.xlsx by matching NIE
    directly. Both sides normalised to plain strings before matching.
    AutoFilter + frozen header row included.
    """
    ...
    # ── 4. Flatten MultiIndex BEFORE merge ───────────────────────
    score_cols_raw = sorted(
        df_pivot.columns.tolist(),
        key=lambda col: (
            0 if 'Matemática' in col[0] else 1,
    ...
    # ── 5. Add Nivel column after each score column ───────────────
    interleaved = []
    for raw_col in score_cols_raw:
        score_col = flat_map[raw_col]
        nivel_col = f"Nivel {raw_col[0]} ({raw_col[1]})"
    ...
    # ── 7. Inject CÓDIGO_SECCIÓN from REQ_027_1 ──────────────────
    df_out = _aplicar_group_id(df_out, lookup_req027)
    ...
    # ── 9. Write with AutoFilter + frozen header ──────────────────
...
if __name__ == "__main__":
    ...
    # 2. REQ_027_1 — NIE column used directly
    ...
    # 4. Cruce con matrícula
    print("\n[*] Cruzando puntajes con la matrícula...")
    df_merged = pd.merge(
        df_all_scores,
        df_matricula[['NIE_Limpio', 'GRUPO_Limpio']],
    ...
    construir_excel_unico(
        df_merged, df_matricula, meses_labels, lookup_req027)
