...
def generar_html_estatico(df_scored, df_mapping, ruta_salida):
    ...
    COLORS_M = [C_DEEP_NAVY, C_STEEL_GREY, C_ROYAL_BLUE]
    ...
    def _bar_items(items, labels, vals, title, color):
        height = max(200, len(items) * 32 + 80)
        ...
        fig.update_layout(
            xaxis_range=[0, 118], height=height, margin=dict(l=_lm(labels), r=80, t=50, b=30),
            plot_bgcolor='white', paper_bgcolor='white', font_color=C_DEEP_NAVY,
            yaxis=dict(tickfont=dict(size=11, color=C_DEEP_NAVY), autorange='reversed', fixedrange=True),
    ...
    # Mapeo para nombres más cortos y limpios en la tabla
    rename_map = {
...
<style>
  body{{font-family:'Segoe UI',sans-serif;margin:0;padding:30px 40px;background:#f0f4f8;}}
  ...
  /* ESTILOS DE LA TABLA (CORRECCIÓN ALINEACIÓN SCROLLX) */
  #tablaEstudiantes {{ border-collapse: collapse; width: 100%; }}
  #tablaEstudiantes th, #tablaEstudiantes td {{ text-align: center; vertical-align: middle; font-size: 12px; white-space: nowrap; }}
  #tablaEstudiantes th {{ background:{C_DEEP_NAVY}; color:#fff; font-size: 11px; padding: 8px 10px; }}
  #tablaEstudiantes tr:hover {{ background:{C_SKY_BLUE}40!important; }}
  
  /* Alinear y expandir "Centro Educativo" (Columna 2) */
  #tablaEstudiantes th:nth-child(2), #tablaEstudiantes td:nth-child(2) {{ 
      text-align: left; 
      min-width: 250px; 
      white-space: normal; 
  }}
  ...
  .toc ul{{list-style:none;padding:0;margin:0;columns:2;gap:30px;}} .toc a{{color:{C_OCEAN_BLUE};text-decoration:none;font-size:13px;}}
</style></head><body>
<h1>📊 Reporte de resultados de la aplicación de la Prueba de Fundamentos</h1>
<div class='container'><div class='metrics'>
...
<div class='toc'><strong>Contenido</strong><ul>
  <li><a href='#sec1'>Sección 1. Distribución global de puntajes</a></li>
  <li><a href='#sec2'>Sección 2. Análisis exhaustivo — Lengua</a></li>
  <li><a href='#sec3'>Sección 3. Análisis exhaustivo — Matemática</a></li>
  <li><a href='#sec4'>Sección 4. Base de datos de estudiantes</a></li>
</ul></div>"""

    # SECCIÓN 1: Global y Grados
    html += "<h2 id='sec1'>Sección 1. Distribución Global de Puntajes por Asignatura y Grado</h2>"
    ...
    for subj_col, subj_name, subj_color in [('L_total_puntaje_0_100', 'Lengua', C_OCEAN_BLUE), ('M_total_puntaje_0_100', 'Matemática', C_DEEP_NAVY)]:
        if subj_col in df_scored.columns and 'Grado' in df_scored.columns:
            ...
            if not df_slice.empty:
                ...
                html += f"<div class='container'>{fig_grado.to_html(full_html=False, include_plotlyjs=False)}</div>"

    # SECCIONES POR BLOQUE (LENGUA)
    html += "<h2 id='sec2'>Sección 2. Análisis Exhaustivo — Lengua</h2><div class='subject-header sh-l'>Diagnóstico Lengua · Nivel 1</div>"
    first_l = False 
    ...
    html += "<h2 id='sec3'>Sección 3. Análisis Exhaustivo — Matemática</h2><div class='subject-header sh-m'>Prueba de Fundamentos Matemáticas</div>"
    ...
    # TABLA FINAL (CON JAVASCRIPT CORREGIDO PARA ALINEACIÓN)
    html += f"""
<h2 id='sec4'>Sección 4. Base de Datos de Estudiantes</h2>
...
$(document).ready(function(){{
    // Clonar la fila de encabezados para los filtros
    $('#tablaEstudiantes thead tr').clone(true).appendTo( '#tablaEstudiantes thead' );
    $('#tablaEstudiantes thead tr:eq(1) th').each( function (i) {{
        var title = $(this).text();
        // Limpiar clases de ordenamiento que DataTables copia por error
        $(this).removeClass('sorting sorting_asc sorting_desc');
        $(this).html( '<input type="text" placeholder="Filtrar" style="width:100%; box-sizing: border-box; font-size: 11px; padding: 4px; border: 1px solid #ccc; border-radius: 4px;" />' );
    ...
    var table = $('#tablaEstudiantes').DataTable({{
        ...
        initComplete: function() {{
            // Forzar a DataTables a recalcular los anchos después de renderizar los inputs
            setTimeout(function() {{
    ...
    // Reajustar anchos si la ventana cambia de tamaño
...
def _ejecutar_etl(current_month_path):
    ...
    if not archivos_form: return None, None, None, None, None
    ...
    # Selección de Matrícula por número de Mes
    ...
    # Mantener Puntuación Global original del Google Forms para el reporte de QC
    ...
    # ==========================================
    # NUEVO: REPORTE DE AUDITORÍA Y CONTROL DE CALIDAD (QC)
    # ==========================================
    ...
    # QC Output
    print("\n" + "="*50 + "\n  🔍 CONTROL DE CALIDAD - UN ESTUDIANTE\n" + "="*50)
    qc = df_scored.iloc[0]
    raw_qc = df_raw.iloc[0]
    print(f"NIE: {qc['NIE']} | Escuela: {qc['Nombre del centro']} | Grado: {qc['Grado']}")
    print("\n--- EJEMPLO DE EXTRACCIÓN Y CALIFICACIÓN (Matemática Bloque 1) ---")
    for item in M_SECCIONES.get('M_Bloque1_Conteo_y_Sentido_Numerico', []):
        if item in raw_qc.index and item in qc.index:
            r_cruda = str(raw_qc[item]).strip()
            r_limpia = r_cruda.lower()
            c_esperada = CLAVE_RESPUESTAS.get(item, '').strip().lower()
            print(f"  {item}: Raw='{r_cruda}' | Limpia='{r_limpia}' | Clave='{c_esperada}' => Puntaje: {qc[item]}")
            
    print(f"\nPuntaje Final: L={qc['L_total_puntaje_0_100']:.1f}% | M={qc['M_total_puntaje_0_100']:.1f}%")
    print("="*50 + "\n")

    df_mapping = pd.DataFrame([{'Item': k, 'Proceso': v} for k, v in TODO_PROCESS_MAP.items()])
    paths = {'raw_df': df_raw, 'scored': df_scored, 'mapping': df_mapping, 'escuelas': df_escuelas, 'qc': df_qc,
    ...
    return df_scored, df_mapping, paths, df_escuelas, df_qc
...
def procesar_y_generar_excel(current_month_path):
    ...
    df_scored, df_mapping, paths, df_escuelas, df_qc = _ejecutar_etl(current_month_path)
    ...
    print("  - 5_Resultados_Escuelas_Fundamentos.xlsx")
    print("  - 6_QC_Auditoria_Puntajes.xlsx")
    return True