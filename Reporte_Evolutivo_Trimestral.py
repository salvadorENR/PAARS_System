...
# ==========================================
# Para Mes 4: agrega el nuevo directorio a DIRECTORIOS_MESES y actualiza
# PATH_OUTPUT_PLOTS. El caché (PATH_CACHE_EXCEL) nunca cambia.
...
DIRECTORIOS_MESES = [
    ...
    # Para Mes 4 descomenta la línea siguiente y actualiza la ruta:
...
# ─── Caché: Excel con los datos ya procesados de los meses anteriores ─────────
# Esta ruta NUNCA cambia entre meses. El archivo se crea la primera vez y
# se va actualizando con cada mes nuevo.
...
# 4. CACHÉ INCREMENTAL
# ==========================================
# El archivo Excel de caché tiene una hoja por cada etiqueta de mes
# (ej. "Mes 1 (Marzo)", "Mes 2 (Abril)", ...).
# Cada hoja guarda exactamente las mismas columnas que producía
# extraer_csvs_puntajes() en el script original, más GRUPO_Limpio.
# Las funciones de graficación NO se modifican: reciben el mismo df
# que antes, construido a partir del caché.
...
def leer_cache():
    """
    Lee todas las hojas del Excel de caché y devuelve un dict
    {etiqueta_mes: DataFrame} con los datos ya procesados de
    meses anteriores. Si el archivo no existe, devuelve {}.
    """
...
def guardar_cache(cache_dict):
    """
    Guarda el diccionario {etiqueta_mes: DataFrame} en el Excel de caché.
    Cada mes es una hoja separada.
    """
    print(f"\n[*] Guardando caché en: {PATH_CACHE_EXCEL}")
    try:
        with pd.ExcelWriter(PATH_CACHE_EXCEL, engine='openpyxl') as writer:
            for mes_label, df in cache_dict.items():
                # El nombre de hoja en Excel tiene límite de 31 chars
                sheet_name = mes_label[:31]
                df.to_excel(writer, sheet_name=sheet_name, index=False)
                print(f"    -> Hoja '{sheet_name}': {len(df):,} filas guardadas")
        print("  [OK] Caché actualizado correctamente.")
    except Exception as e:
...
def extraer_csvs_mes(ruta, etiqueta_mes, df_matricula, mapa_grupos_escuela):
    """
    Lee los CSVs de UN solo mes, cruza con matrícula y asigna GRUPO_Limpio.
    Devuelve un DataFrame con las columnas COLS_CACHE.
    """
    lista_dfs = []
    ...
    # Cruce con matrícula y asignación de GRUPO_Limpio
    df_mes = pd.merge(df_mes, df_matricula[['NIE_Limpio']],
                      left_on='Documento', right_on='NIE_Limpio', how='inner')
    df_mes['GRUPO_Limpio'] = (
        df_mes['Nro de Centro'].map(mapa_grupos_escuela).fillna('DESCONOCIDO'))

    print(f"  [OK] {etiqueta_mes}: {len(df_mes):,} registros leídos de CSVs")
    return df_mes[COLS_CACHE]


# ==========================================
# 6. CONTEOS Y ALERTAS (SIN FILTROS)
# ==========================================
def generar_reportes_conteos_separados(df_base_total):
    print("\n[*] Generando Reportes de Totales de Estudiantes (Sin filtros)...")
    grados_esperados = list(range(2, 12))
    meses_labels     = sorted(df_base_total['Mes'].unique(),
                              key=lambda m: list(df_base_total['Mes'].unique()).index(m))
    ...
    if len(meses_labels) >= 2:
        ...
        if not df_b2_temprano.empty:
            ...
            for _, row in conteo.iterrows():
                print(f"      - {row['Nro de Centro']} - {row['Centro']} "
                      f"| {row['Materia']} | {row['Mes']}: {row['Documento']} estudiante(s)")
        else:
            print("  [OK] Ninguna escuela B2 fue evaluada en Mes 1 o Mes 2.")
    else:
        print("  [-] No hay suficientes meses para esta validación.")


# ==========================================
# 7. SANKEY  (sin cambios respecto al original)
# ==========================================
...
def generar_kde_y_boxplot(df_master, meses_labels, sufijo_archivo="B1",
                          color_list=None):
...
def exportar_base_consolidada_b1(df_master, df_matricula, lookup_req027, meses_labels):
    ...
    try:
        ...
        for raw_col in score_cols_raw:
            sc = flat_map[raw_col]; nc = f"Nivel {raw_col[0]} ({raw_col[1]})"
...
# ==========================================
# 11. B2 EXCEL + BARRAS  (sin cambios)
# ==========================================
def generar_excel_y_barras_b2(df_b2_mes3, df_matricula, lookup_req027):
    if df_b2_mes3.empty:
        print("\n  [!] No hay estudiantes en el Grupo B2 para este mes.")
        return
    ...
    for base_col in ['Puntaje Matemática', 'Puntaje Lengua']:
        ...
        df_pivot[base_col.replace('Puntaje', 'Nivel')] = (
            df_pivot[base_col].apply(clasificar_puntaje))
    ...
    df_excel[final_cols].to_excel(
        os.path.join(dir_xls_b2, "Resultados_B2_MesActual.xlsx"),
        index=False)
...
# ==========================================
# 12. EXCEL MAESTRO  (sin cambios)
# ==========================================
def construir_excel_unico(df_merged, df_matricula, meses_labels, lookup_req027):
    print("\n[*] Construyendo Excel único (una sola hoja, todos los datos)...")
    ...
    if df_all.empty:
        print("  [!] No hay datos para el Excel único."); return None
    ...
    print(f"  [OK] Excel único → {out_path} ({len(df_out):,} filas, {size_mb:.1f} MB)")
    return out_path
...
if __name__ == "__main__":
    ...
    # ── 2. Leer caché y decidir qué meses hay que leer de CSV ─────
    print("\n[*] Verificando caché de datos...")
    cache = leer_cache()                          # {etiqueta_mes: DataFrame}
    meses_en_cache = set(cache.keys())
    meses_a_leer   = [m for m in meses_labels if m not in meses_en_cache]

    if meses_a_leer:
        print(f"  [*] Meses nuevos a leer de CSVs: {meses_a_leer}")
    else:
        print("  [*] Todos los meses ya están en caché — se omite la lectura de CSVs.")

    # ── 3. Leer solo los meses que no están en caché ──────────────
    for ruta, etiqueta in DIRECTORIOS_MESES:
        if etiqueta not in meses_a_leer:
            continue
        print(f"\n[*] Leyendo CSVs: {etiqueta}")
        df_mes = extraer_csvs_mes(ruta, etiqueta, df_matricula, mapa_grupos_escuela)
        if not df_mes.empty:
            cache[etiqueta] = df_mes
        else:
            print(f"  [!] Sin datos para {etiqueta} — no se agrega al caché.")

    # ── 4. Guardar caché actualizado ──────────────────────────────
    if meses_a_leer:
        # Solo guardar si hubo meses nuevos
        cache_a_guardar = {m: cache[m] for m in meses_labels if m in cache}
        guardar_cache(cache_a_guardar)

    # ── 5. Construir df_merged_full combinando todos los meses ────
    meses_disponibles = [m for m in meses_labels if m in cache]
    if not meses_disponibles:
        print("\n[!] Sin datos disponibles — proceso detenido."); sys.exit()
    ...
    # ── 7. Filtros de trayectoria y gráficas B1 ───────────────────
    print("\n[*] Aplicando filtros de trayectoria para gráficas...")
    ...
    if not df_b2.empty:
        ...
        if len(meses_b2) >= 2:
            dibujar_sankey_multimes(df_b2, "Matemática", meses_b2, sufijo_sankey="B2")
            dibujar_sankey_multimes(df_b2, "Lengua",     meses_b2, sufijo_sankey="B2")
        else:
        ...
        generar_kde_y_boxplot(df_b2, meses_labels, sufijo_archivo="B2",
                              color_list=["#b3b3ff", "#9b9bff", "#7b7bdf", "#5b5bbf"])

        # Barras por grado y todos los grados — lista global para numeración correcta
